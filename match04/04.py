...
def parse(data):
    ...
    content = base64.b64encode(
        (data['key'] + data['value']).encode()).replace(b'=', b'')
    j_key = hashlib.md5(content).hexdigest()
    # print(j_key)
    soup = BeautifulSoup(data['info'], 'lxml')
    tds = soup.select('td')
    for td in tds:
        imgs = td.select('img')
        i = 0
        outnumber = ['', '', '', '']
        for img in imgs:
            if not j_key in img['class']:
                number = numbersMap[img['src'].replace('data:image/png;base64,','')]
                matchStyleNum = re.search('left:(.*?)px',img['style'])
                if matchStyleNum:
                    styleNum = float(matchStyleNum.group(1))/11.5
                    outnumber[i + int(styleNum)] = number
                i += 1
        value = ''.join(outnumber)
        scrapeData.append(value)
    return scrapeData
