...
发现请求的逻辑在最后一行，复制出来格式化  
![](./pic/01/02.png)


发现m参数在这里出现了，oo0O0(timestamp.toString()) + window.f;  
```
window.url = '/api/match/1';
// 时间戳进行了偏移
var timestamp = Date.parse(new Date()) + 100000000;
// oo0O0函数的返回值是空字符串 尝试hook window.f
...
发现oo0O0函数的返回值为空字符串 可以尝试hook window.f
```
function oo0O0(mw) {
    window.b = '';
...
在浏览器调试结果  
![](../success_pictures/success_01_1.png)

python + JavaScript代码调试结果
![](../success_pictures/success_pictures01.png)